...
class ImageReshaper (picamera.array.PiMotionAnalysis):
    ...
    def write(self, data):
        self.image = np.reshape(np.fromstring(
            data, dtype=np.uint8), (240, 320, 3))
...
def main():
    ...
    image_pub = rospy.Publisher(
        "/walkerbotweb/image_raw", Image, queue_size=1, tcp_nodelay=False)
                ...
                while not rospy.is_shutdown():
                    ...
                    if image_reshaper.previous_image is not None:
                        image_message = bridge.cv2_to_imgmsg(
                            image_reshaper.previous_image, encoding="bgr8")
